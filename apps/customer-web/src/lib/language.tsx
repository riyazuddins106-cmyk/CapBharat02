...
export function LanguageProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(() => {
    try {
      return detectLocale(localStorage.getItem(STORAGE_KEY) || 'en');
    } catch {
      return 'en';
    }
  });
  ...
  const value = useMemo(() => ({
    ...
    tx: (source: string) => translateText(locale, source),
  ...
  return (
    <label className="inline-flex items-center gap-2 text-xs font-semibold text-gray-500">
      ...
      <select
        ...
        className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 outline-none focus:border-violet-400"
      >
        {enabledLocales.map((item) => <option key={item} value={item}>{LOCALE_LABELS[item]}</option>)}
      </select>
    </label>
  );
}